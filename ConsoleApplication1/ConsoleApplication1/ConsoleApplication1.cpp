...
int main(int argc, char* argv[]) {
    ...
    if (param_date == "-" && param_datedate == "-") {
        ...
        if (curl) {
            ...
            if (res != CURLE_OK) {
                std::cerr << "Ошибка при загрузке: " << curl_easy_strerror(res) << std::endl;
            }
            ...
            for (pugi::xml_node valute : doc.child("ValCurs").children("Valute")) {
                ...
                if (charCode == param_vname) {
                    ...
                    if (param_vnom != "-") {
                        ...
                        cout << t << '\n';
                        double value = std::stod(value_str) * conversionRate;
                        cout << value << '\n';
        ...
        if (curl) {
            ...
            for (pugi::xml_node valute : doc.child("ValCurs").children("Valute")) {
                std::string charCode = valute.child_value("CharCode");
                if (charCode == param_vname) {
                    std::string value = valute.child_value("Value");
                    std::string nominal = valute.child_value("Nominal");

                    if (param_vnom != "-") {
                        // Проверяем, если параметр vnom не равен "-", то пересчитываем значение value и меняем nominal
                        double conversionRate = std::stod(param_vnom) / std::stod(nominal);
                        value = std::to_string(std::stod(value) * conversionRate);
                        nominal = param_vnom;
                    }

                    outputInfo = param_vname + ' ' + nominal + ' ' + date + ' ' + value; // Сохраняем значение и номинал в outputInfo
        ...
        if (param_filename != "-") {
            ...
            if (outputFile.is_open()) {
                for (const auto& answer : answers) {
                    outputFile << answer << '\n'; // Записываем информацию с новой строки
                }
            }
            else {
                std::cerr << "Ошибка открытия файла для записи." << std::endl;
            ...
            for (const auto& answer : answers) {
                std::cout << answer << std::endl; // Выводим информацию на экран с новой строки
            }
        }
    }
    return 0;
